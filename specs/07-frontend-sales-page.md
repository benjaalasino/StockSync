# 07 — Frontend: página Ventas (`/sales`)

## Archivo a crear: `frontend/src/pages/SalesPage.tsx`

La página tiene dos vistas: lista de ventas y un drawer/modal para crear una nueva venta.

El flujo de creación tiene 3 pasos en un solo modal:
1. **Seleccionar cliente** (con búsqueda y opción de crear uno nuevo inline)
2. **Agregar ítems** (variante + cantidad + precio)
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {sales.map(sale => {
                const total = sale.items?.reduce((s, i) => s + i.quantity * i.unit_price, 0) ?? 0
                return (
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (err?.response?.status === 409) {
        setSubmitError(detail || 'Stock insuficiente para uno o más ítems')
      } else {
        setSubmitError(detail || 'Error al registrar la venta')
      }
    } finally {
      setSubmitting(false)
    }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 pt-5 pb-4 border-b border-gray-100">
          ...
          <div className="flex gap-2 text-xs">
            {(['1. Cliente', '2. Ítems', '3. Confirmar'] as const).map((label, i) => (
              <span
                key={i}
                className={`px-3 py-1 rounded-full ${step === i + 1 ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'}`}
              >
                {label}
              </span>
            ))}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {step === 2 && (
            <StepItems items={items} onChange={setItems} />
          )}
          {step === 3 && (
            <StepConfirm
...
function StepClient({ selected, onSelect }: { selected: Client | null; onSelect: (c: Client) => void }) {
    ...
    <div className="space-y-4">
      ...
      {selected ? (
        <div className="flex items-center justify-between bg-blue-50 border border-blue-200 rounded-lg px-4 py-3">
          ...
          <button onClick={() => onSelect(null as any)} className="text-xs text-blue-500 hover:underline">Cambiar</button>
        </div>
      ) : (
        <>
          <input
...
function InlineClientCreate({ onSave, onCancel }: { onSave: (d: ClientCreate) => void; onCancel: () => void }) {
...
function StepItems({ items, onChange }: { items: SaleItemDraft[]; onChange: (items: SaleItemDraft[]) => void }) {
  ...
  useEffect(() => {
    if (selectedVariant) setPrice(selectedVariant.sale_price)
  }, [selectedVariantId])
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-[1fr_80px_120px_auto] gap-2 items-end">
        ...
        <button
          onClick={addItem}
          className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 mt-5"
        >
          Agregar
        </button>
      </div>
...
function StepConfirm({
  client, items, total, notes, onNotesChange, error
}: {
  client: Client | null
  items: SaleItemDraft[]
  total: number
...
## Verificar que `salesApi` tenga el método `list`

Ver spec `04-frontend-types-api.md` — si `GET /stock/sales` no existe en el backend, crearlo primero.
