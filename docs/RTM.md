...
| REQ-F06  | Gestión de Clientes (CRUD) y asociación a Ventas  | ✅     | 11 tests |
...
## REQ-F06 — Gestión de Clientes (CRUD) y asociación a Ventas

> El sistema debe permitir gestionar un listado de clientes (crear, leer, actualizar, eliminar)
> y asociar una venta a un cliente existente.

### Código fuente

| Archivo                          | Líneas | Función/Clase                                    | Rol                                                                     |
| -------------------------------- | ------ | ------------------------------------------------ | ----------------------------------------------------------------------- |
| `app/models/client.py`           | 10–30  | `class Client`                                   | Modelo de Cliente                                                       |
| `app/models/stock.py`            | 135    | `Sale.client_id`                                 | FK a `clients.id`                                                       |
| `app/schemas/client.py`          | 8–32   | `ClientCreate`, `ClientUpdate`, `ClientResponse` | Schemas Pydantic                                                        |
| `app/services/client_service.py` | 15–80  | `ClientService`                                  | Lógica de negocio CRUD                                                  |
| `app/services/stock_service.py`  | 135    | `create_sale()`                                  | Asigna `client_id` a la venta                                           |
| `app/api/v1/clients.py`          | 18–75  | `router`                                         | Endpoints `GET /`, `POST /`, `GET /{id}`, `PATCH /{id}`, `DELETE /{id}` |
| `app/api/v1/stock.py`            | 78     | `create_sale()`                                  | Recibe `client_id` opcional                                             |

### Tests

| Archivo                           | Función                                     | Verifica                                       |
| --------------------------------- | ------------------------------------------- | ---------------------------------------------- |
| `tests/test_api_clients.py:22`    | `test_tc33_crear_cliente_admin`             | TC-33: Admin crea cliente → 201                |
| `tests/test_api_clients.py:38`    | `test_tc34_crear_cliente_operador_falla`    | TC-34: Operator crea cliente → 403             |
| `tests/test_api_clients.py:53`    | `test_tc35_crear_cliente_email_duplicado`   | TC-35: Email duplicado → 400                   |
| `tests/test_api_clients.py:69`    | `test_tc36_listar_clientes_admin`           | TC-36: Admin lista clientes → 200              |
| `tests/test_api_clients.py:85`    | `test_tc37_obtener_cliente_por_id`          | TC-37: Admin obtiene un cliente → 200          |
| `tests/test_api_clients.py:101`   | `test_tc38_actualizar_cliente_admin`        | TC-38: Admin actualiza cliente → 200           |
| `tests/test_api_clients.py:117`   | `test_tc39_eliminar_cliente_logico`         | TC-39: Admin borra cliente → `is_active=False` |
| `tests/test_client_service.py:15` | `test_delete_client_service_sets_inactive`  | Servicio marca `is_active=False`               |
| `tests/test_api_stock.py:81`      | `test_tc40_crear_venta_con_cliente`         | TC-40: Venta se asocia a `client_id`           |
| `tests/test_api_stock.py:99`      | `test_tc41_crear_venta_cliente_inexistente` | TC-41: Venta con `client_id` inválido → 404    |
| `tests/test_api_stock.py:115`     | `test_tc42_crear_venta_sin_cliente`         | TC-42: Venta sin `client_id` es válida → 201   |
...
| REQ-F06   | 11     | `test_api_clients.py`, `test_client_service.py`, `test_api_stock.py`                                               |
